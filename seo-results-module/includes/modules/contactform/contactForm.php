...
# @Last modified time: 2019-02-13T13:11:33-08:00
# NOTE: Code made with help from this thread: https://github.com/elegantthemes/create-divi-extension/issues/20

class SERM_ContactForm extends ET_Builder_Module {
	...
	function init() {
		...
		$this->slug            = 'serm_contact_form';
		$this->vb_support      = 'on';
		$this->child_slug      = 'et_pb_contact_field';
		$this->child_item_text = esc_html__( 'Field', 'et_builder' );

		$this->main_css_element = '%%order_class%%.serm_contact_form_container';
		...
		$this->advanced_fields = array(
			...
			'max_width'             => array(
				'css' => array(
					'module_alignment' => '%%order_class%%.serm_contact_form_container.et_pb_module',
				),
			),
			'text'                  => array(
				'css' => array(
		...
		$this->custom_css_fields = array(
			...
			'contact_button' => array(
				...
				'selector' => '.serm_contact_form_container .et_contact_bottom_container .et_pb_contact_submit.et_pb_button',
	...
	function render( $attrs, $content = null, $render_slug ) {
		global $et_pb_half_width_counter, $serm_contact_form_num;
		...
		$serm_contact_form_num = $this->render_count();

		$content = $this->content;

		$et_error_message = '';
		$et_contact_error = false;
		$current_form_fields = isset( $_POST['et_pb_contact_email_fields_' . $serm_contact_form_num] ) ? $_POST['et_pb_contact_email_fields_' . $serm_contact_form_num] : '';
		$hidden_form_fields = isset( $_POST['et_pb_contact_email_hidden_fields_' . $serm_contact_form_num] ) ? $_POST['et_pb_contact_email_hidden_fields_' . $serm_contact_form_num] : false;
		$contact_email = '';
		$processed_fields_values = array();

		$nonce_result = isset( $_POST['_wpnonce-et-pb-contact-form-submitted-' . $serm_contact_form_num] ) && wp_verify_nonce( $_POST['_wpnonce-et-pb-contact-form-submitted-' . $serm_contact_form_num], 'et-pb-contact-form-submit' ) ? true : false;

		// check that the form was submitted and et_pb_contactform_validate field is empty to protect from spam
		if ( $nonce_result && isset( $_POST['et_pb_contactform_submit_' . $serm_contact_form_num] ) && empty( $_POST['et_pb_contactform_validate_' . $serm_contact_form_num] ) ) {
			if ( '' !== $current_form_fields ) {
				$fields_data_json = str_replace( '\\', '' ,  $current_form_fields );
				$fields_data_array = json_decode( $fields_data_json, true );

				// check whether captcha field is not empty
				if ( 'on' === $captcha && ( ! isset( $_POST['et_pb_contact_captcha_' . $serm_contact_form_num] ) || empty( $_POST['et_pb_contact_captcha_' . $serm_contact_form_num] ) ) ) {
			...
			if ( false === $nonce_result && isset( $_POST['et_pb_contactform_submit_' . $serm_contact_form_num] ) && empty( $_POST['et_pb_contactform_validate_' . $serm_contact_form_num] ) ) {
		...
		$et_pb_captcha = sprintf( '
			...
			esc_attr( $serm_contact_form_num ),
		...
		if ( $et_contact_error ) {
			...
			$form = sprintf( '
				<div class="et_pb_contact">
					<form class="serm_contact_form clearfix" method="post" action="%1$s">
						%8$s
				...
				wp_nonce_field( 'et-pb-contact-form-submit', '_wpnonce-et-pb-contact-form-submitted-' . $serm_contact_form_num, true, false ),
		...
		$module_id = '' !== $this->module_id(false) ? $this->module_id(false) : 'serm_contact_form_' . $serm_contact_form_num;
		...
		$output = sprintf( '
			<div id="%4$s" class="%5$s" data-form_unique_num="%6$s"%7$s>
				...
				%3$s
			</div> <!-- .serm_contact_form_container -->
			',
